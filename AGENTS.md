...
**照片追加与去重.** `loadPhotos` (file picker *and* drag-and-drop) **appends** to `state.images`;
it never replaces the set, so existing indices keep their meaning and `crops`/`picks` survive
untouched — only 清空图片 (`clearImage`) wipes them. A file already in the list is skipped, keyed by
`photoKey(name, size, lastModified)` (`File` metadata only, the bytes are never hashed); duplicates
*within* one batch are skipped too. `state.imageMeta` is the parallel record — same order, same
length as `state.images`, entries `{blob, name, key}`, which is also exactly the IndexedDB item
shape, so persisting is one `idbPut('grid', { items: state.imageMeta })` of the whole list rather
than a read-modify-write. Keep the two arrays in lockstep: anything that drops an image (a failed
decode in `loadPhotos`/`restoreImages`/`importScheme`) must drop its meta entry in the same pass —
that is why those three decode into `{it, img}` pairs and filter the pair, not the image list alone.
A record written before this change has no `key`; `itemKey` re-derives one from `name` + the blob's
size/`lastModified`, so an old profile dedups correctly without a migration step. The info line under
the button reports **only the count** (plus `（跳过 N 张重复）` for the batch just added) — it
deliberately does not name the single-image file.
...
the photos come back — the same policy as an orphaned crop key or an out-of-bounds merge. Only
clearing (`clearImage`) resets `picks` alongside `crops`; `loadPhotos` **appends** and so leaves
both alone — see 照片追加 below. `importScheme`
